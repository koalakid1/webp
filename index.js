...
exports.handler = async (event, context, callback) => {
  ...
  //type 이 비어있는 경우 변환할 필요 없이 요청 그대로 처리
  if (type === null || type === "" || type != "webp" || params.length === 0) {
    console.log("type 변환할 필요 없음");
    try {
      const originParams = {
        Bucket: BUCKET,
      ...
      const imageObject = await S3.getObject(originParams).promise();
      etag = imageObject.ETag;
      resizedImage = await Sharp(imageObject.Body).toBuffer();
      ...
      response.headers["content-disposition"] = [
        {
          ...
          value: `attachment; filename=${imageName + "." + extension}`,
        },
      ];
